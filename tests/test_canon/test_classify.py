...
- `TestSelfPairsAreRejectedBeforeTheModelSeesThem` -- a self-loop survived the
  first measured run and shipped in its fabrication sample.
- `TestOneRelationPerPair` -- pins the single-answer contract, which a measured
  and reverted experiment briefly widened to two.
"""
...
class TestSelfPairsAreRejectedBeforeTheModelSeesThem:
    ...
    and shipped in its fabrication sample; chapter 4 carried three such edges and
    two reached the output. No relation in this ontology says anything by
    relating a thing to itself, and the failure class has been known on this
    project since its first fabrication check."""
    ...
    def test_the_match_is_on_the_shared_name_fold(self):
        ...
        subject = pair(a_name="Strahd", a_type="NPC", b_name="  strahd ", b_type="NPC")

        assert is_self_pair(subject)
    ...
    @pytest.mark.asyncio
    async def test_the_askable_pairs_still_line_up_after_a_self_pair_is_skipped(self):
        self_pair = pair(a_name="Helga", a_type="NPC", b_name="Helga", b_type="NPC")
        client = make_client({"answers": [
        ...
        decisions = await classifier.classify([self_pair, pair()])

        assert decisions[0] == Decision("", "", NONE_RELATION, "")
        assert decisions[1] == Decision("Ismark", "Vallaki", "TRAVELED_TO", "clear")


class TestOneRelationPerPair:
    """A version allowing two was built, measured and reverted: it was used ZERO
    times in 374 opportunities while its prompt rewrite cost 8.4 points of
    decline rate and broke one of the five diagnosed failures. These tests pin
    the single-answer contract that came back."""

    @pytest.mark.asyncio
    async def test_one_pair_yields_exactly_one_decision(self):
        client = make_client({"answers": [
            {"n": 1, "answer": "Ismark -TRAVELED_TO-> Vallaki", "confidence": "clear"}
        ]})
        classifier = RelationClassifier(client=client, model="test-model")
    ...
    def test_the_prompt_asks_for_at_most_one(self):
