...
def offered_options(pair: Pair) -> list[tuple[str, str, RelationshipType]]:
    ...
    A self-pair yields each type once -- the two directions are the same option.
    """
...
class RelationClassifier:
    ...
    async def classify(self, pairs: list[Pair]) -> list[Decision]:
        ...
        for index, pair in enumerate(pairs):
            if offered_options(pair):
                askable.append((index, pair))
                continue
            # No type admits any relation between these two, in either
            # direction. There is nothing to offer, so there is nothing to ask.
